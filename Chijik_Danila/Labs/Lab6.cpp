...
Разработать функцию, возвращающую сумму значений узлов самой длинной ветви дерева.
*/
typedef struct node
{
  int numb;
...
int a;
char aa;
lp:printf("Есть несколько ветвей одинаковой длины. По какой Вы хотите пойти?\n 1 - по левой\n -1 - по правой\n ");
fflush (stdin);
	 if(scanf("%d",&a)!=1||((aa=getchar())!='\n')||(a!=1&&a!=-1))
  {
	  fflush (stdin);
	  printf("Ошибка при выборе пути\n");
	  goto lp;
  }
  return a;
}
